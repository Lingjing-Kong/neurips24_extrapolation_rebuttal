...
def setup_experiment(opt):
    ...
    opt.dist=False
    if torch.cuda.is_available():
        opt.dist = opt.nproc_per_node > 1
        opt.world_size = opt.nproc_per_node

        if opt.dist:
            init_distributed_mode(opt)
            opt.batch_size = opt.batch_size//opt.world_size
        else:
            opt.rank, opt.gpu = 0, 0
...
def find_subfolder_matching(root_path, substring):
    matching_folders = []

    # Walk through all directories and subdirectories in the given path
    for path, subdirs, files in os.walk(root_path):
        # Check each subdirectory name for the substring
        for name in subdirs:
...
def determine_to_run(group_name, exp_name, root_path):
    
    exp_paths = find_subfolder_matching(root_path=join(root_path, group_name), substring=exp_name)
