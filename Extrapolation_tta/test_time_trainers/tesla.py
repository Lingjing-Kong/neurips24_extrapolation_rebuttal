...
from utilities.utils import ensure_dir, normalize_fn, denormalize_fn, offline, determine_to_run
from networks.ema import ModelEMA
from networks.lora import LoRANetwork
from utilities.show_results import WANDBLogger
...
class TeSLA(object):
    def __init__(self, opt):
        ...
        # test
        if not determine_to_run(
            group_name=opt.exp_group, exp_name=opt.exp_name, root_path="../wandb",
        ):  
            print(f"{opt.exp_group}---{opt.exp_name} is skipped!!!")
            sys.exit()
        ...
        self.run = wandb.init(project="tta_extrapolation", group=opt.exp_group, name=opt.exp_name, config=opt)
        folder_name = self.opt.exp_name + "_" + self.run.id if os.environ['WANDB_MODE'] == "online" else self.opt.exp_name # permit overwriting test folders
        self.opt.checkpoint_path = os.path.join("./wandb", self.opt.exp_group, folder_name)
        Path(self.opt.checkpoint_path).mkdir(parents=True, exist_ok=True)
        self.run.config.update({"checkpoint_path": self.opt.checkpoint_path})

        # logger
        self.logger = WANDBLogger(checkpoint_path=self.opt.checkpoint_path, wandb_run=self.run)
        ...
        # source model
        opt.lora_rank = [int(rank) for rank in opt.lora_rank.split(",")] if "," in opt.lora_rank else int(opt.lora_rank)
        self.net = build_network(
            network_name=opt.arch, 
            n_classes=opt.n_classes, 
            ckpt_path=opt.pretrained_source_path, 
        )
        ...
        if use_lora:
            ...
            self.forward_model = self.lora
        else:
            self.forward_model = self.net

        # Load source stats if we load them
        if self.opt.use_source_stats :
            if "cifar" in opt.dataset_name:
                ...
                self.ext_src_mu = [mu.half() for mu in self.ext_src_mu]
                self.ext_src_cov = [cov.half() for cov in self.ext_src_cov]
                self.template_ext_cov = self.template_ext_cov.half()

                self.ext_src_mu = torch.stack(self.ext_src_mu)
                self.ext_src_cov = torch.stack(self.ext_src_cov) + self.template_ext_cov[None, :, :]

                # Move to half precision
                self.ema_n = torch.zeros(self.opt.n_classes, dtype=torch.float16).cuda()
                self.ema_ext_mu = self.ext_src_mu.clone()
                self.ema_ext_cov = self.ext_src_cov.clone()

                # Store in CPU if not used immediately
                self.ema_ext_total_mu = torch.zeros(self.feat_dim).half().cpu()
                self.ema_ext_total_cov = torch.zeros(self.feat_dim, self.feat_dim).half().cpu()
        ...
        self.ema_net = ModelEMA(self.forward_model, decay=opt.ema_momentum, lora_decay=opt.ema_momentum_).cuda()

        # Parallel Processing
        if opt.dist:
            raise NotImplementedError("Not implementet yet.")
        ...
        lora_params, main_params = self.lora.prepare_optimizer_params()

        assert len(lora_params) + len(main_params) == len(list(self.forward_model.parameters()))
        param_groups = [
            {"params": lora_params, "basel_lr": self.opt.lr * self.opt.lora_lr_factor, "lr": self.opt.lr * self.opt.lora_lr_factor},
        ]
        param_groups.append(
            {"params": main_params, "basel_lr": self.opt.lr},
        )

        if self.opt.dataset_name == "imagenet" in self.opt.dataset_name:
            self.optimizer_net = torch.optim.SGD(param_groups, momentum=0.9, lr=self.opt.lr, weight_decay=self.opt.wd)
        else:
            self.optimizer_net = torch.optim.AdamW(param_groups, lr=self.opt.lr, weight_decay=self.opt.wd)
    ...
    def evaluate(self, net, data_loader, epoch, title_label="Target"):
        ...
        for x, label, _ in data_loader:
            ...
            logit, feat = net(x[:,0].to("cuda", non_blocking=True), return_feats=True)
    ...
    def lr_scheduler(self, optimizer, iter_num, max_iter, gamma=10, power=0.75):
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = param_group['base_lr'] * decay
        return optimizer
    ...
    def train(self):
        ...
        while n_epochs <= self.opt.n_epochs:
            ...
            for i, (x, label, _) in enumerate(self.target_loader):
                ...
                with torch.no_grad():
                    ...
                    for i in reversed(range(x.size(1))):
                        logit, feat = self.ema_net(x[:,i].cuda(), return_feats=True)
                        score = self.softmax(logit / self.opt.softmax_temperature)
                    ...
                    if self.opt.nn_queue_size > 0:
                        ...
                        self.update_nearest_neighbours(concat_all_gather(feats_ema_easy, n_processes=self.opt.nproc_per_node), concat_all_gather(soft_pseudo_labels, n_processes=self.opt.nproc_per_node))
                ...
                scores_normal, feats_normal = self.forward_model(batch_x, return_feats=True)
                scores_normal = self.softmax(scores_normal / self.opt.softmax_temperature)
                ...
                if self.opt.pl_ce:
                    ...
                    loss_cm_cent  = self.crterian_cm(concat_all_gather(scores_normal, rank=self.opt.rank, n_processes=self.opt.nproc_per_node))*self.opt.world_size
                    loss_teach_normal = self.cross_entropy_loss(scores_normal, soft_pseudo_labels).mean()

                # sparsity
                sparse_loss = torch.tensor([0.]).cuda()
                p_total = 0
                for n, p in self.forward_model.named_parameters():
                    if "gate" in n or "soft_mask" in n:
                        sparse_loss += torch.sum(torch.abs(p))
                        p_total += torch.numel(p.data)
                sparse_loss = self.opt.sparsity_weight * sparse_loss / p_total

                loss = loss_cm_cent.item() + loss_teach_normal.item() + sparse_loss.item()
                ...
                if self.opt.use_source_stats:
                    ...
                    (loss_cm_cent + loss_teach_normal + sparse_loss + loss_stats).backward()
                else:
                    (loss_cm_cent + loss_teach_normal + sparse_loss).backward()

                # teacher student knowledge distillation
                feats_hard = None
                if not self.opt.no_kl_hard:
                    for j in range(self.opt.aug_mult):
                        logits_hard, feats_hard = self.forward_model(x_aug_hard[:, j], return_feats=True)
                        scores_hard = self.softmax(logits_hard / self.opt.softmax_temperature)
                ...
                if self.opt.dist:
                    ...
                    self.ema_net.update(self.forward_model)
                ...
                if self.opt.n_epochs == 1:
                    with torch.no_grad():
                        ...
                        student_pred, feats_pred = self.forward_model(x[:,0].cuda(), return_feats=True)
                        student_pred = self.softmax(student_pred)
                        self.forward_model.train()

                        # teacher pred
                        self.ema_net.eval()
                        teacher_pred = self.ema_net(x[:,0].cuda())
                        teacher_pred = self.softmax(teacher_pred)
                        self.ema_net.train()
            ...
            log_file_path = os.path.join(self.opt.experiment_dir, "log.txt")  # Define the log file path
            ...
            if self.opt.n_epochs == 1:
                ...
                if self.opt.rank==0:
                    ...
                    online_acc_teacher = 100 * online_acc_teacher.to(torch.float32).mean().item()
                    online_acc_student = self.metadata_tracker["Label"]==self.metadata_tracker["Pred_student"]
                    online_acc_student = 100 * online_acc_student.to(torch.float32).mean().item()
                    online_acc_plr = self.metadata_tracker["Label"]==self.metadata_tracker["Pred_PLR"]
                    online_acc_plr = 100 * online_acc_plr.to(torch.float32).mean().item()
                    ...
                    with open(log_file_path, "a") as log_file:  # Open the log file in append mode
                        log_file.write(f"Epoch {n_epochs} - Online Evaluation\n")
                        log_file.write(f"Online Teacher accuracy/error : %4.4f/%4.4f\n" % (online_acc_teacher, 100-online_acc_teacher))
                        log_file.write(f"Online Students accuracy/error : %4.4f/%4.4f\n" % (online_acc_student, 100-online_acc_student))
                        log_file.write(f"Online PLR accuracy/error : %4.4f/%4.4f\n\n" % (online_acc_plr, 100-online_acc_plr))

                    # self.run.log(
                    self.logger(
                        {
                            "epoch": n_epochs,
                            "teacher_accuracy": online_acc_teacher,
                            "student_accuracy": online_acc_student,
                            "plr_accuracy": online_acc_plr,
                        }
                    )
                ...
                acc = self.evaluate(self.forward_model, self.eval_target_loader, n_epochs, title_label="Student")
                print(f"Student accuracy/error : %4.4f/%4.4f\n" % (acc, 100-acc))

                with open(log_file_path, "a") as log_file:  # Open the log file in append mode
                    log_file.write(f"Epoch {n_epochs} - Offline Evaluation\n")
                    log_file.write(f"Teacher accuracy/error : %4.4f/%4.4f\n" % (acc, 100-acc))
                    log_file.write(f"Student accuracy/error : %4.4f/%4.4f\n\n" % (acc, 100-acc))

            if self.opt.save_models and ((self.opt.save_every > 0 and n_epochs % self.opt.save_every == 0) or n_epochs==self.opt.n_epochs):
                # Save the network
                torch.save(self.forward_model.state_dict(), os.path.join(self.opt.experiment_dir, f"SLAug_Net_Epoch[{n_epochs}].pth"))
