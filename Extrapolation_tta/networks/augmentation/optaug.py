...
class OptAug(nn.Module):
    ...
    def optimize(self, x_cuda):
        ...
        for k in range(self.aug_mult):
            ...
            loss_policy = torch.mean(loss_curr * torch.log(curr_selected_prob_sub_policy + 1e-6))
            loss_policy.backward()

            # Check gradients for NaN/Inf values
            for name, param in self.policy_predictor.named_parameters():
                if param.grad is not None:
                    if check_nan_inf(param.grad, f"Gradient of {name}"):
                        # Implement your strategy here (e.g., reduce learning rate, stop training, etc.)
                        assert False
        ...
        # Check gradients for NaN/Inf values
        for name, param in self.policy_predictor.named_parameters():
            if check_nan_inf(param, f"Weight of {name}"):
